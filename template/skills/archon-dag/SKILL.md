...
description: |
  ...
  Triggers (api): "archon api", "archon rest", "workflow api", "run workflow via api",
            "start pipeline", "trigger pipeline".
  Capability: Runs AI workflows in isolated git worktrees for parallel development.
  Also: Creates and manages workflow YAML files, commands, and configuration.
  REST API on port 3090 for programmatic workflow management.
  NOT for: Direct Claude Code work - only for delegating to Archon CLI.
  NOT for: Knowledge/RAG search (V1 feature, archived in archive/v1 branch).
argument-hint: "[workflow] [message or issue number]"
---

# Archon V2 Workflow Engine Skill

> **Version note**: This skill covers Archon V2 (v0.3.10+), the Bun/TypeScript DAG workflow engine.
> The V1 Python knowledge/RAG system (`/api/knowledge-items/*`, port 8181) is archived on
> `archive/v1-task-management-rag`. Do NOT reference V1 endpoints.

Archon V2 is a DAG workflow engine that runs AI coding workflows in isolated git worktrees.
It has a full REST API on port 3090 for programmatic access.

## Our Setup

- **Binary**: `/usr/local/bin/archon` (v0.3.10)
- **Web UI + API**: `http://localhost:3090`
- **Project**: `~/archon-video-generation-workflow/`
- **DB**: `~/.archon/archon.db` (SQLite)
- **Worktrees**: `~/.archon/workspaces/ubuntu/archon-video-generation-workflow/worktrees/`
- **Custom workflows**: `remotion-idea-to-video`, `remotion-idea-to-video-pipboy`, `remotion-from-hn`, `remotion-from-hn-pi`
- **API script**: `~/.archon/scripts/archon-api.sh` (see references/api-reference.md)
- **All 3 custom workflows validate OK with `archon validate workflows`**
